...
// World coords: -10..10. We render into a square viewport.
type Props = { size: number };

export function Compass({ size }: Props) {
  const { x, y, trail } = useCompass();
  ...
  const toPx = (wx: number, wy: number) => ({
    px: ((wx + 10) / 20) * size,
    py: ((10 - -wy) / 20) * size, // invert: positive y (autoritário) goes UP
  });
  // explanation: world y positive = autoritário (top). So screen y = (10 - y)/20 * size
    ...
    >
      ...
      {ideologies.map((i) => {
        ...
        return (
          ...
          >
            <span className="absolute inset-0 -translate-x-1/2 -translate-y-1/2 left-1/2 top-1/2 rounded-full pulse-ring"
              style={{ width: 22, height: 22, background: 'transparent', border: `1px solid ${i.color}`, opacity: isHover ? 0.9 : 0.5 }} />
            <span className="block rounded-full transition-all"
              style={{
                width: isHover ? 14 : 10, height: isHover ? 14 : 10,
                background: i.color,
                boxShadow: `0 0 ${isHover ? 24 : 12}px ${i.color}, 0 0 4px ${i.color}`,
              }} />
            <span className="absolute left-3 top-3 text-[9px] font-mono tracking-wider whitespace-nowrap text-foreground/70 group-hover:text-foreground"
              style={{ textShadow: `0 0 8px ${i.color}` }}>
              {i.short.toUpperCase()}
            </span>
          </button>
        );
      ...
      <AnimatePresence>
        {hover && (
          <motion.div
            ...
            className="absolute glass-strong rounded-md p-4 max-w-sm pointer-events-none hud-corner"
            style={{
              left: clampPx(mouse.mx + 18, size, 380),
              top: clampPx(mouse.my + 18, size, 280),
